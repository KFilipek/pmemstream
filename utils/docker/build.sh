...
#		prepared for running builds and tests. It uses Docker image
...
#	[and possibly user/org name, and package name], e.g. "<CR_addr>/pmem/pmemstream",
...
CONTAINER_NAME=pmemstream-${OS}-${OS_VER}
...
if [[ -z "${COMMAND}" ]]; then
	...
	case ${TYPE} in
	debug)
		builds=(tests_gcc_debug_cpp17_no_valgrind
	...
	release)
		...
		;;
	valgrind)
		builds=(tests_gcc_debug_cpp17_valgrind)
		COMMAND="./run-build.sh ${builds[@]}";
		;;
	package)
